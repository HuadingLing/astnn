...
func main() {
	...
	if *versionFlag {
		...
		if err != nil {
			Perish(TOPLEVEL_POSITION, "path_error", err.Error())
		}
		info, err := file.Stat()
		if err != nil {
			Perish(TOPLEVEL_POSITION, "path_error", err.Error())
		...
		if err != nil {
			Perish(INVALID_POSITION, "positionless_syntax_error", err.Error())
		}
		...
		val, _ := json.Marshal(TestExpr(*exprFlag))
		os.Stdout.Write(val)
	} else if *stmtFlag != "" {
		val := TestStmt(*stmtFlag)
		os.Stdout.Write(val)
	} else {
		flag.PrintDefaults()
	}
