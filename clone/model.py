...
class BatchProgramCC(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        return self.predict(lvec, rvec)
        #return lvec, rvec
        
    def predict(self, lvec, rvec):
        #abs_dist = torch.abs(torch.add(lvec, -rvec))  # r=|r1-r2|, r.shape = (batch_size, 2*hidden_dim)， 为啥用1范数？
        #y = torch.sigmoid(self.hidden2label(abs_dist))  # 论文中公式(9)的p
        
        return F.pairwise_distance(lvec, rvec, keepdim=True)
